...
pub fn repr_from_bytes<const N: usize>(inputs: &[u8]) -> [u64; N] {
    let mut repr: [u64; N] = [0; N];
    for (i, &byte) in inputs.iter().enumerate() {
        repr[i / 8] |= (byte as u64) << (8 * (i % 8));
    }
    repr
}

pub fn bytes_from_repr<const N: usize>(inputs: &[u64]) -> [u8; N] {
    let mut bytes: [u8; N] = [0; N];
    let mut tmp: u64 = 0;
    for i in 0..min(inputs.len(), N*8) {
        if i % 8 == 0 {
            tmp = inputs[i / 8];
        }
        bytes[i] = tmp as u8;
